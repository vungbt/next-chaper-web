...
type SelectAsyncCreatableProps = Omit<SelectProps, 'isClearable'> & {
  ...
  delay?: number;
  filterOptions?: (searchValue?: string) => IOptItem[];
};

export const SelectAsyncCreatable = forwardRef(function SelectAsyncCreatable(
  props: SelectAsyncCreatableProps,
  ref: Ref<never>
) {
  const {
    className,
        ...
        >
          <RenderIcon
            ...
            className={clsx('text-dark transition-all ease-linear', {
              'rotate-180 text-green-700': menuIsOpen,

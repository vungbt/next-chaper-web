...
export default function ThemeSwitcher() {
  ...
  useEffect(() => {
    const themeMode = localStorageHelper.get(EKeyStorage.THEME_MODE);
    if (!themeMode) return changeTheme(getSystemTheme());
    changeTheme(themeMode as EThemeMode);
    if (themeMode === EThemeMode.DARK) return setIsDarkMode(true);
    setIsDarkMode(false);
  }, []);
  ...
  const onHandleToggle = () => {
    ...
    if (!isDarkMode) return changeTheme(EThemeMode.DARK);
    changeTheme(EThemeMode.LIGHT);
  };
